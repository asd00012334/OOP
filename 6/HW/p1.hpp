...
class AVLTree {
    ...
    friend int heightCheck(Node* const& ThisNode){
        if(ThisNode==nullptr) return 0;
        else return ThisNode->Height;
    }
...
auto AVLTree<key_type, mapped_type, key_compare>::Node::Find(Node* Tree, const key_type& KeyToFind, key_compare& Comp) -> Node* {
    ...
    while(1){
        ...
        if(Comp((CurrentNode->Mapping).first,KeyToFind))
            CurrentNode=CurrentNode->Right;
...
auto AVLTree<key_type, mapped_type, key_compare>::Node::RightRotate(Node* ThisNode) noexcept -> Node* {
    ...
    auto b=ThisNode, a=ThisNode->Left,
    B=ThisNode->Left->Right, Source=ThisNode->Parent;

    if(Source!=nullptr)
        if(b==Source->Left) Source->Left=a;
        else Source->Right=a;
    ...
    b->Height=std::max(heightCheck(b->Left),heightCheck(b->Right))+1;
    a->Height=std::max(heightCheck(a->Left),heightCheck(a->Right))+1;
    if(Source!=nullptr)
        Source->Height=std::max(heightCheck(Source->Left),heightCheck(Source->Right))+1;
...
auto AVLTree<key_type, mapped_type, key_compare>::Node::LeftRotate(Node* ThisNode) noexcept -> Node* {
	...
	//       The height of modified trees should also be fixed accordingly.
    auto Source=ThisNode->Parent,
    a=ThisNode, b=ThisNode->Right,
    B=ThisNode->Right->Left;
    if(Source!=nullptr)
        if(Source->Left==a) Source->Left=b;
        else Source->Right=b;
